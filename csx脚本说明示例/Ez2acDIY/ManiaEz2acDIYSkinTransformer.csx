...
// ═══════════════════════════════════════════════════════════════════════════
// 配置类 — 所有自定义参数集中在此，方便维护
// ═══════════════════════════════════════════════════════════════════════════

private static class Ez2acDIYConfig
{
    // ── 配置表 1：列宽配置 ──
    // 格式：new KeyColumnConfig(列宽, [Scratch列索引], 因子, [Pedal列索引], 因子, [Hidden列索引], 因子)

    public static readonly Dictionary<int, KeyColumnConfig> keyConfigs = new()
    {
        [4]  = new KeyColumnConfig(64f, 20.48f),
        [5]  = new KeyColumnConfig(64f, 20.48f),
        [6]  = new KeyColumnConfig(64f, 20.48f),
        [7]  = new KeyColumnConfig(64f, 20.48f),
        [8]  = new KeyColumnConfig(60f, 19.2f),
        [9]  = new KeyColumnConfig(52f, 16.64f),
        [10] = new KeyColumnConfig(48f, 15.36f),
        [12] = new KeyColumnConfig(48f, 15.36f, new[] { 0, 11 }, 1.2f, 1.2f),
        [14] = new KeyColumnConfig(48f, 15.36f, new[] { 0, 12 }, 1.2f, 1.2f, new[] { 6 }, 1.2f, 1.2f, new[] { 13 }, 0f, 0f),
        [16] = new KeyColumnConfig(40f, 12.8f, new[] { 0, 15 }, 1.2f, 1.2f),
        [18] = new KeyColumnConfig(36f, 11.52f, new[] { 0, 16 }, 1.2f, 1.2f, new[] { 8 }, 1.2f, 1.2f, new[] { 17 }, 0f, 0f),
    };

    /// <summary>未知 Key 数时的回退配置</summary>
    public static readonly KeyColumnConfig defaultKeyConfig = new(64f);

    // ── 配置表 2：列类型映射 ──
    // A/E → "white"(白), B → "blue"(蓝), S/P → "green"(绿)

    public static readonly Dictionary<int, EzColumnType[]> columnTypeConfigs = new()
    {
    ...
    public static EzColumnType[] GetColumnTypes(int keyMode)
        => columnTypeConfigs.TryGetValue(keyMode, out var types) ? types : System.Array.Empty<EzColumnType>();

    /// <summary>获取指定 Key 数的列配置，未注册的回退到 defaultKeyConfig</summary>
    public static KeyColumnConfig ResolveConfig(int totalColumns)
        => keyConfigs.TryGetValue(totalColumns, out var cfg) ? cfg : defaultKeyConfig;

    // ── 路径工具 ──

    public static string GetKeyModeDir(int columns) => columns switch
    {
        4 => "fourkey",
        5 => "fivekey",
        6 => "sixkey",
    ...
    public const float hitPositionValue = 110f;      // 判定线距底部（px）
    public const int stage_padding_bottom = 0;
}
...
public struct KeyColumnConfig
{
    public float ColumnWidth;
    public float NoteHeight;
    public int[]? ScratchColumnIndices;
    public float ScratchFactor;
    public float NoteScratchFactor;
    public int[]? PedalColumnIndices;
    public float PedalFactor;
    public float NotePedalFactor;
    public int[]? HiddenColumnIndices;
    public float HiddenFactor;
    public float NoteHiddenFactor;

    public KeyColumnConfig(float columnWidth, float noteHeight = 30f,
        int[]? scratchColumnIndices = null, float scratchFactor = 1f, float noteScratchFactor = 1f,
        int[]? pedalColumnIndices = null, float pedalFactor = 1f, float notePedalFactor = 1f,
        int[]? hiddenColumnIndices = null, float hiddenFactor = 0f, float noteHiddenFactor = 0f)
    {
        ColumnWidth = columnWidth;
        NoteHeight = noteHeight;
        ScratchColumnIndices = scratchColumnIndices;
        ScratchFactor = scratchFactor;
        NoteScratchFactor = noteScratchFactor;
        PedalColumnIndices = pedalColumnIndices;
        PedalFactor = pedalFactor;
        NotePedalFactor = notePedalFactor;
        HiddenColumnIndices = hiddenColumnIndices;
        HiddenFactor = hiddenFactor;
        NoteHiddenFactor = noteHiddenFactor;
    }
}

// ═══════════════════════════════════════════════════════════════════════════
// Ez2acDIYColumnBackground — 加载后覆写 Column.EzNoteSizeBindable
// 所有 Note 绘制逻辑完全沿用 EzPro 原始 EzNote / EzHoldNoteHead / EzHoldNoteTail / EzHoldNoteMiddle
// ═══════════════════════════════════════════════════════════════════════════

public partial class Ez2acDIYColumnBackground : EzColumnBackground
{
    [Resolved]
    ...
    private void overrideColumnType()
    {
        Scheduler.Add(() =>
        {
            var types = Ez2acDIYConfig.GetColumnTypes(column.KeyMode);
    ...
    private void overrideNoteSize()
    {
        Scheduler.AddOnce(() =>
        {
            ...
            // 从 Config 查当前 Key 数的 NoteHeight，再乘列系数
            var cfg = Ez2acDIYConfig.ResolveConfig(column.KeyMode);
            float noteFactor = 1f;
            if (cfg.ScratchColumnIndices?.Contains(column.Index) == true)
                noteFactor = cfg.NoteScratchFactor;
            else if (cfg.PedalColumnIndices?.Contains(column.Index) == true)
                noteFactor = cfg.NotePedalFactor;
            else if (cfg.HiddenColumnIndices?.Contains(column.Index) == true)
                noteFactor = cfg.NoteHiddenFactor;

            float h = cfg.NoteHeight * noteFactor;
...
// ═══════════════════════════════════════════════════════════════════════════
// Ez2acDIYStageBackground — 从 Stage/{keyCount}key/Body.png 加载
// ═══════════════════════════════════════════════════════════════════════════

public partial class Ez2acDIYStageBackground : CompositeDrawable
{
    [Resolved]
    private ISkinSource skin { get; set; } = null!;
    ...
    public Ez2acDIYStageBackground()
    {
    ...
    private void load()
    {
        var tex = skin.GetTexture($"Stage/{Ez2acDIYConfig.GetKeyModeDir(stageDefinition.Columns)}/Body");
        if (tex != null)
        {
            InternalChild = body = new Sprite
            {
                Texture = tex,
    ...
    {
        base.Update();
        if (body == null || sizeApplied) return;
        if (DrawWidth <= 0) return;

        float scale = DrawWidth / 412f;
        body.Scale = new Vector2(scale);
        body.Y = 205f - 384f * scale + 180f - Ez2acDIYConfig.hitPositionValue;
        sizeApplied = true;
    }
}

// ═══════════════════════════════════════════════════════════════════════════
// Ez2acDIYKeyArea — 从 Stage/{keyCount}key/keypress/{n}KeyPress_{state}.png 加载
// ═══════════════════════════════════════════════════════════════════════════

public partial class Ez2acDIYKeyArea : CompositeDrawable, IKeyBindingHandler<ManiaAction>
{
    [Resolved]
    private Column column { get; set; } = null!;
    ...
    private Sprite pressSprite = null!;
    private int maxState;

    public Ez2acDIYKeyArea()
    {
        RelativeSizeAxes = Axes.Both;
    }
    ...
    private void load()
    {
        string keyDir = Ez2acDIYConfig.GetKeyModeDir(column.KeyMode);
        int col = column.Index;
        maxState = column.KeyMode >= 8 ? 2 : 1;

        pressSprite = new Sprite
        {
            Texture = skin.GetTexture($"Stage/{keyDir}/keypress/{col + 1}KeyPress_0"),
            Anchor = Anchor.BottomCentre,
            Origin = Anchor.BottomCentre,
    ...
    public bool OnPressed(KeyBindingPressEvent<ManiaAction> e)
    {
        ...
        string keyDir = Ez2acDIYConfig.GetKeyModeDir(column.KeyMode);
        int col = column.Index;
        var tex = skin.GetTexture($"Stage/{keyDir}/keypress/{col + 1}KeyPress_{maxState}");
        if (tex != null) pressSprite.Texture = tex;
        pressSprite.FadeTo(1, 70);
        return false;
    }
    ...
    public void OnReleased(KeyBindingReleaseEvent<ManiaAction> e)
    {
        ...
        string keyDir = Ez2acDIYConfig.GetKeyModeDir(column.KeyMode);
        int col = column.Index;
        var tex = skin.GetTexture($"Stage/{keyDir}/keypress/{col + 1}KeyPress_0");
        if (tex != null) pressSprite.Texture = tex;
        pressSprite.FadeTo(0.3f, 800);
    }
}
...
public class ManiaEz2acDIYTransformer : SkinTransformer
{
    private readonly ManiaBeatmap beatmap;

    public ManiaEz2acDIYTransformer(ISkin skin, IBeatmap beatmap)
        : base(skin)
    {
        this.beatmap = (ManiaBeatmap)beatmap;
        // 不再从 Ez2ConfigManager 读取任何配置 —— 全部使用 Ez2acDIYConfig 中的硬编码常量
    }
            ...
            case ManiaSkinComponentLookup maniaComponent:
                switch (maniaComponent.Component)
                {
                    case ManiaSkinComponents.ColumnBackground:
                        return new Ez2acDIYColumnBackground();
                    case ManiaSkinComponents.KeyArea:
                        return new Ez2acDIYKeyArea();
                    ...
                    case ManiaSkinComponents.StageBackground:
                        return new Ez2acDIYStageBackground();
                    ...
                    case GlobalSkinnableContainers.MainHUDComponents:
                        return new DefaultSkinComponentsContainer(container =>
                        {
                            ...
                            if (keyCounter != null)
                            {
                                ...
                                keyCounter.Position = new Vector2(0, -Ez2acDIYConfig.hitPositionValue - Ez2acDIYConfig.stage_padding_bottom);
                            }
                            ...
                            if (columnHitErrorMeter != null)
                            {
                                ...
                                columnHitErrorMeter.Position = new Vector2(0, -Ez2acDIYConfig.hitPositionValue - Ez2acDIYConfig.stage_padding_bottom);
                            }
    ...
    private static KeyColumnConfig resolveConfig(int totalColumns)
        => Ez2acDIYConfig.keyConfigs.TryGetValue(totalColumns, out var cfg) ? cfg : Ez2acDIYConfig.defaultKeyConfig;
        ...
        if (lookup is ManiaSkinConfigurationLookup maniaLookup)
        {
            ...
            switch (maniaLookup.Lookup)
            {
                ...
                case LegacyManiaSkinConfigurationLookups.HitPosition:
                    return SkinUtils.As<TValue>(new Bindable<float>(Ez2acDIYConfig.hitPositionValue));
