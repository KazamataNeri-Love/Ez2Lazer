...
// ═══════════════════════════════════════════════════════════════════════════
// Ez2acDIYJudgementHub — 全判定数据中转站（不渲染）
// ═══════════════════════════════════════════════════════════════════════════
// 注入 JudgementCountController，收集所有 IsBasic() 判定的 BindableInt，
// 通过 public 属性暴露给其他组件绑定。自身不绘制任何内容。
// ═══════════════════════════════════════════════════════════════════════════

public class Ez2acDIYJudgementHub : Component
{
    [Resolved(canBeNull: true)]
    private JudgementCountController? judgementCountController { get; set; }

    /// <summary>HitResult → 实时计数的 BindableInt 映射</summary>
    public IReadOnlyDictionary<HitResult, BindableInt> Counts => counts;
    private readonly Dictionary<HitResult, BindableInt> counts = new Dictionary<HitResult, BindableInt>();
    ...
    protected override void LoadComplete()
    {
        ...
        foreach (var counter in judgementCountController.Counters)
        {
            ...
            counts[key] = counter.ResultCount;
        }
    }
}
...
public class Ez2acDIY : Skin
{
    public static SkinInfo CreateInfo() => new SkinInfo
    {
        ...
        Creator = "V.A.",
        Protected = true,
        InstantiationInfo = typeof(Ez2acDIY).GetInvariantInstantiationInfo()
    };

    protected EzResourceStore? EzResources;

    public Ez2acDIY(IStorageResourceProvider resources)
        : this(CreateInfo(), resources)
    {
    }

    [UsedImplicitly(ImplicitUseKindFlags.InstantiatedWithFixedConstructorSignature)]
    public Ez2acDIY(SkinInfo skin, IStorageResourceProvider resources, IResourceStore<byte[]>? fallbackStore = null)
    ...
    {
        // 优先读取皮肤目录自身的资源
        Texture? texture = Textures?.Get(componentName, wrapModeS, wrapModeT);
        if (texture != null)
            return texture;

        // 回退到 EzResources（游戏全局资源）
        return EzResources?.Get(componentName);
    }
