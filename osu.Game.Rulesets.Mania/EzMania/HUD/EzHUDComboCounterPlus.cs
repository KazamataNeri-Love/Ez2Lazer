...
{
    /// <summary>
    /// 逐字符 Sprite 渲染的 Combo 计数器。
    /// FillFlowContainer 自动排列居中，缩放动画作用于整个容器。
    /// Spacing 通过 TransformTo 与 Scale 同步缓动，放大时数字向外扩散。
    /// </summary>
    public partial class EzHUDComboCounterPlus : CompositeDrawable, ISerialisableDrawable
    {
        ...
        {
            float currentScale = charFlow.Scale.X;
            float targetScale = Math.Clamp(currentScale * (wasIncrease ? EffectStartFactor.Value : EffectEndFactor.Value), 0.5f, 3f);

            // Spacing 位移峰值 = 字符宽度 × 缩放增量 × 位移倍数
            float peakSpacing = refCharWidth * (targetScale - 1f) * DisplacementMultiplier.Value;

            // 缩放 + Spacing 用 TransformTo 同步缓动
            charFlow
